...
const universities = graduates.map(elem => elem.university);
...
const contactInfo = graduates.map(elem => `${elem.first_name} ${elem.email}`);
console.log(contactInfo);


/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all. Log the result. */
const uni = graduates.map(elem => elem.university).filter(elem => elem.includes("Uni"));
console.log(uni);
...
*/
const populationTotal = zooAnimals.reduce((acc, elem) => acc += elem.population, 0);
